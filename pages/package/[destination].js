...
import { getDestinationName, getThemeColors, getHeroImage, getPackageImages, getPackageData, getPackageRoute } from '@/utils/packageHelpers';
...
export default function PackagePage() {
  ...
  useEffect(() => {
    if (router.isReady) {
      setIsLoading(false);
      if (!destination) setError('Destination not specified');
    }
  ...
  const packageImages = getPackageImages(category);
  const packages = getPackageData(destinationName, packageImages);
  const filteredPackages = packages.filter(pkg => pkg.price <= budget);

  const handlePackageClick = () => {
    const packageId = getPackageRoute(destinationName);
          ...
          <Grid item xs={12} md={3}>
            <Card sx={{ padding: '2rem', borderRadius: '20px', boxShadow: '0 8px 32px rgba(0, 0, 0, 0.1)', position: 'sticky', top: '100px' }}>
              ...
              <Box sx={{ padding: '0 0.5rem', mb: 2 }}>
                <Slider value={budget} onChange={(e, newValue) => handleBudgetChange(newValue)} min={8000} max={40000} step={1000} valueLabelDisplay="auto" valueLabelFormat={(value) => `₹${value.toLocaleString()}`} sx={{ color: themeColors.primary, '& .MuiSlider-thumb': { backgroundColor: themeColors.primary } }} />
          ...
          <Grid item xs={12} md={9}>
            ...
            <Grid container spacing={2.5}>
              {filteredPackages.map((pkg, index) => (
                <Grid item xs={12} sm={6} md={4} key={index}>
                  <Card onClick={handlePackageClick} sx={{ borderRadius: '16px', overflow: 'hidden', transition: 'all 0.3s ease', cursor: 'pointer', height: '100%', '&:hover': { transform: 'translateY(-8px)', boxShadow: `0 16px 32px ${themeColors.primary}40` } }}>
                    <Box sx={{ height: 180, backgroundImage: `url(${pkg.image})`, backgroundSize: 'cover', backgroundPosition: 'center' }} />
                    <CardContent sx={{ padding: '1.25rem' }}>
                      <Typography variant="h6" sx={{ fontWeight: 700, mb: 0.4, color: muiTheme.palette.text.primary, fontSize: '1.1rem' }}>{pkg.title}</Typography>
                      <Typography variant="body2" sx={{ color: themeColors.primary, mb: 0.75, fontWeight: 600 }}>{pkg.duration}</Typography>
                      <Box sx={{ mb: 1 }}>
                        {pkg.features.slice(0, 3).map((feature, idx) => (
                          <Chip key={idx} label={feature} size="small" sx={{ margin: '0.2rem', backgroundColor: `${themeColors.primary}15`, color: themeColors.primary, fontWeight: 500, fontSize: '0.7rem', height: '22px' }} />
                        ))}
                      </Box>
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <Typography variant="h5" sx={{ fontWeight: 700, color: themeColors.primary, fontSize: '1.5rem' }}>₹{pkg.price.toLocaleString()}</Typography>
            ...
            {filteredPackages.length === 0 && (
              <Box sx={{ textAlign: 'center', padding: '4rem 2rem', backgroundColor: muiTheme.palette.background.paper, borderRadius: '20px' }}>
                ...
                <Typography variant="body1" sx={{ color: muiTheme.palette.text.secondary, mb: 2 }}>Try increasing your budget to ₹{Math.min(...packages.map(p => p.price)).toLocaleString()} or more</Typography>
