    ...
    try {
      await initiatePayment({
        ...
        onFailure: (error) => {
          ...
          if (error.message.includes("Payment gateway is currently unavailable")) {
            alert("⚠️ Payment Gateway Not Configured\n\nThe payment system is currently unavailable. This happens when Razorpay credentials are not set up.\n\nPlease contact the administrator or try again later.");
          } else if (error.message === "Failed to load Razorpay SDK" || error.message.includes("fetch")) {
            alert("Cannot connect to payment server. Please make sure the backend server is running.");
          } else {
